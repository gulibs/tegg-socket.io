...
- 统一的部署方案，通过框架和平台的双向控制，应用只需要关注自己的代码，具体查看[应用部署](https://eggjs.org/zh-CN/core/deployment)。
- 统一的代码风格，框架不仅仅解决代码重用问题，还可以对应用做一定约束，作为企业框架是很必要的。Egg 在 Koa 的基础上做了很多约定，框架可以使用 [Loader](https://eggjs.org/zh-CN/advanced/loader) 自己定义代码规则。
...
具体可以参见[渐进式开发](https://eggjs.org/zh-CN/intro/progressive)。

## 框架与多进程

框架的扩展与多进程模型有关，我们已经了解了[多进程模型](https://eggjs.org/zh-CN/core/cluster-and-ipc)，也知道 `Agent Worker` 和 `App Worker` 的区别。因此，我们需要扩展的类也有两个：`Agent` 和 `Application`，这两个类的 API 不一定相同。

在 `Agent Worker` 启动的时候会实例化 `Agent`，而在 `App Worker` 启动时会实例化 `Application`。这两个类又同时继承自 [EggCore](https://github.com/eggjs/egg-core)。

EggCore 可以看做是 Koa `Application` 的升级版，默认内置了 [Loader](https://eggjs.org/zh-CN/advanced/loader)、[Router](https://eggjs.org/zh-CN/basics/router) 及应用异步启动等功能，可以看作是支持 `Loader` 的 Koa。
...
现在，yadan 框架的目录已经成为了一个加载单元（loadUnit），因此相应的目录和文件（如 `app` 和 `config`）都会被加载。详情可以查看[框架被加载的文件](https://eggjs.org/zh-CN/advanced/loader)。
...
以上都是示例代码，用于解释框架路径加载过程。实际上，Egg 已经提供了[本地开发](https://eggjs.org/zh-CN/core/development)和[应用部署](https://eggjs.org/zh-CN/core/deployment)的优秀工具，不需要我们自行实现。 下面是根据《优秀技术文档的写作标准》修改后的全文内容：
...
框架启动过程在[多进程模型](https://eggjs.org/zh-CN/core/cluster-and-ipc)、[Loader](https://eggjs.org/zh-CN/advanced/loader)、[插件](https://eggjs.org/zh-CN/advanced/plugin)中或多或少都有提及。这里我们系统地梳理一下启动顺序：
...
在看下文之前，请先查看[单元测试章节](https://eggjs.org/zh-CN/core/unittest)。框架测试的大部分使用场景和应用类似。
