...
在[本地开发](https://eggjs.org/zh-CN/core/development)时，我们使用 `egg-bin dev` 来启动服务，但在部署应用时不可以这样使用。因为 `egg-bin dev` 会针对本地开发做很多处理，而生产环境需要一个更加简单稳定的方式。本章主要讲解如何部署你的应用。
...
框架内置 [egg-cluster](https://github.com/eggjs/egg-cluster) 启动 [Master 进程](https://eggjs.org/zh-CN/core/cluster-and-ipc#master)，Master 稳定，不需 [pm2](https://github.com/Unitech/pm2) 等进程守护模块。

框架同时提供 [egg-scripts](https://github.com/eggjs/egg-scripts) 支持线上运行和停止。
...
- `--framework=yadan`：使用[自定义框架](https://eggjs.org/zh-CN/advanced/framework)时，配置 `package.json` 的 `egg.framework` 或指定该参数。
- `--ignore-stderr`：忽略启动期错误。
- `--https.key`：HTTPS 密钥路径。
- `--https.cert`：HTTPS 证书路径。

[egg-cluster](https://github.com/eggjs/egg-cluster) 的所有 Options 支持透传，如 `--port` 等。

更多参数见 [egg-scripts](https://github.com/eggjs/egg-scripts) 和 [egg-cluster](https://github.com/eggjs/egg-cluster) 文档。
...
[nodeinstall](https://github.com/cnpm/nodeinstall)会把对应版本的 `alinode` 安装到项目的 `node_modules` 目录下。

> 注意：打包机的操作系统和线上系统需保持一致，否则对应的 Runtime 不一定能正常运行。

#### 安装及配置

我们提供了[egg-alinode](https://github.com/eggjs/egg-alinode)来快速接入，无需安装 `agenthub` 等额外的常驻服务。
