...
框架内置了 `@eggjs/view` 作为模板解决方案，支持多模板渲染。每个模板引擎均以插件方式引入，并保持渲染 API 的一致性。如想深入了解，可查看[模板插件开发](https://eggjs.org/zh-CN/advanced/view-plugin)。
...
在模板中可以直接使用 `helper` 上注册的方法，具体可以参见[扩展](https://eggjs.org/zh-CN/basics/extend)。
...
框架内置的 [@eggjs/security](https://github.com/eggjs/egg/tree/master/plugins/security) 插件，提供了常见的安全辅助函数，包括 `helper.shtml`、`surl`、`sjs` 等，强烈建议阅读安全性相关的[文档内容](https://eggjs.org/zh-CN/core/security)。