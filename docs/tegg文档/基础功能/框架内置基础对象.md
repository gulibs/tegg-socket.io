...
Application 是全局应用对象。在一个应用中，每个进程只会实例化一个 Application 实例。它继承自 `Koa.Application`，在其上我们可以挂载一些全局的方法和对象。我们可以轻易地在插件或应用中[扩展 Application 对象](https://eggjs.org/zh-CN/basics/extend#application)。

### 事件

在框架运行时，会在 Application 实例上触发一些事件，应用开发者或插件开发者可以监听这些事件做一些操作。作为应用开发者，我们一般会在[启动自定义脚本](https://eggjs.org/zh-CN/basics/app-start)中进行监听：
...
- [启动自定义脚本](https://eggjs.org/zh-CN/basics/app-start)
...
- [Controller 文件](https://eggjs.org/zh-CN/basics/controller)
...
在[定时任务](https://eggjs.org/zh-CN/basics/schedule)中，每个 task 都接收一个 Context 实例作为参数，这样我们可以更便利地执行一些定时的业务逻辑：
...
- `config` - 应用的[配置](https://eggjs.org/zh-CN/basics/config)。
- `service` - 应用的所有 [service](https://eggjs.org/zh-CN/basics/service)。
...
Service 基类的属性和 [Controller](https://eggjs.org/zh-CN/basics/objects#controller) 基类属性一致，访问方式也类似：
...
除此之外，Helper 的实例还可以在模板中获取。例如，可以在模板中使用 [security](https://eggjs.org/zh-CN/core/security) 插件提供的 `shtml` 方法。
...
在应用开发中，我们可能经常需要自定义一些 Helper 方法。例如上面例子中的 `formatUser`，我们可以通过 [框架扩展](https://eggjs.org/zh-CN/basics/extend#helper) 的形式来自定义 Helper 方法。
...
我们推荐应用开发遵循配置和代码分离的原则，将一些需要硬编码的业务配置都放到配置文件中。同时，配置文件支持各个不同的运行环境使用不同的配置，使用起来也非常方便。所有框架、插件和应用级别的配置都可以通过 `Config` 对象获取到。关于框架的配置，可以详细阅读[Config 配置](https://eggjs.org/zh-CN/basics/config)章节。
...
框架内置了功能强大的[日志功能](https://eggjs.org/zh-CN/core/logger)，可以非常方便地打印各种级别的日志到对应的日志文件中，每一个 logger 对象都提供了 4 个级别的方法：
...
- [中间件（middleware）](https://eggjs.org/zh-CN/basics/middleware)
- [控制器（controller）](https://eggjs.org/zh-CN/basics/controller)
- [服务（service）](https://eggjs.org/zh-CN/basics/service)