...
框架支持根据环境来加载配置，定义多个环境的配置文件，具体环境请查看[运行环境配置](https://eggjs.org/zh-CN/basics/env)。
...
**注意**：插件之间也会有加载顺序，但大致顺序类似。具体逻辑可[查看加载器](https://eggjs.org/zh-CN/advanced/loader)。
...
1. 安全字段，如密码、密钥等。这些字段通过 `config.dump.ignore` 属性进行配置，其类型必须是 [Set](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Set)。可参见[默认配置](https://github.com/eggjs/egg/blob/master/config/config.default.js)。
