...
在这之前，TJ 的 [co](https://github.com/tj/co) 使我们可以提前享受到 `async/await` 的编程体验，但同时它不可避免地也带来了一些问题：
...
- 只支持 Node.js 8 及以上版本。
- 去除 [co](https://github.com/tj/co) 后堆栈信息更清晰，带来 30% 左右的性能提升（不含 Node 带来的性能提升），详细参见：[benchmark](https://eggjs.github.io/benchmark/plot/)。

Egg 的理念之一是“渐进式增强”，故我们为开发者提供“渐进升级”的体验。

- [快速升级](https://eggjs.org/zh-CN/intro/migration#快速升级)
- [插件变更说明](https://eggjs.org/zh-CN/intro/migration#插件变更说明)
- [进一步升级](https://eggjs.org/zh-CN/intro/migration#进一步升级)
- [针对“插件开发者”的升级指南](https://eggjs.org/zh-CN/intro/migration#插件升级)
...
- 修改为推荐的代码风格，传送门：[代码风格指南](https://eggjs.org/zh-CN/community/style-guide)
- [中间件使用 Koa2 风格](https://eggjs.org/zh-CN/intro/migration#中间件使用-koa2-风格)
- [将 `yieldable` 函数调用转为 `awaitable`](https://eggjs.org/zh-CN/intro/migration#yieldable-to-awaitable)
...
- 返回的函数入参改为 Koa 2 的
  ...
  - 第一个参数为 `ctx`，代表当前请求的上下文，是 [Context](https://eggjs.org/zh-CN/basics/extend#context) 的实例。
...
[co](https://github.com/tj/co) 支持了 `yieldable` 兼容类型：
...
如果无法修改相关接口，可以暂时使用我们提供的工具方法 [app.toPromise](https://github.com/eggjs/egg-core/blob/da4ba1784175c43217125f3d5cd7f0be3d5396bf/lib/egg.js#L353) 兼容一下。
...
只需改写为相应的 async 函数即可。如果无法修改，可以使用 [app.toAsyncFunction](https://github.com/eggjs/egg-core/blob/da4ba1784175c43217125f3d5cd7f0be3d5396bf/lib/egg.js#L344) 方法简单包装一下。

**注意**

- 使用 [toAsyncFunction](https://github.com/eggjs/egg-core/blob/da4ba1784175c43217125f3d5cd7f0be3d5396bf/lib/egg.js#L344) 和 [toPromise](https://github.com/eggjs/egg-core/blob/da4ba1784175c43217125f3d5cd7f0be3d5396bf/lib/egg.js#L353) 实际上是借助 [co](https://github.com/tj/co) 进行包装的，因此可能导致性能损失和堆栈问题。我们建议开发者尽可能进行全链路升级。
- 在调用 async 函数时，[toAsyncFunction](https://github.com/eggjs/egg-core/blob/da4ba1784175c43217125f3d5cd7f0be3d5396bf/lib/egg.js#L344) 不会引起额外损失。
