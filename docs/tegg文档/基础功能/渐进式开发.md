...
在 Egg 里面，有[插件](https://eggjs.org/zh-CN/basics/plugin)和[框架](https://eggjs.org/zh-CN/advanced/framework)，前者还包括了 `path` 和 `package` 两种加载模式，那我们应该如何选择呢？
...
首先，我们抽出一个 `egg-ua` 插件，看过[插件文档](https://eggjs.org/zh-CN/advanced/plugin)的同学应该都比较熟悉，我们这里只简单过一下：
...
首先，抽象出 `example-framework` 框架，如上看过[框架文档](https://eggjs.org/zh-CN/advanced/framework)的同学应该都比较熟悉，我们这里只简单过一下：
