...
还是通过[快速入门](https://eggjs.org/zh-CN/intro/quickstart)章节介绍的 `npm` 来初始化我们的应用：
...
首先，我们先按照前面的设计来注册[路由](https://eggjs.org/zh-CN/basics/router)，框架提供了一个便捷的方式来创建 RESTful 风格的路由，并将一个资源的接口映射到对应的 controller 文件。在 `app/router.js` 中编写：
...
在 [controller](https://eggjs.org/zh-CN/basics/controller) 中，我们只需要实现 `app.resources` 约定的 [RESTful 风格的 URL 定义](https://eggjs.org/zh-CN/basics/router#restful-风格的-url-定义) 中我们需要提供的接口即可。例如我们来实现创建一个 `topics` 的接口：
...
在 [service](https://eggjs.org/zh-CN/basics/service) 中，我们可以更加专注地编写实际生效的业务逻辑。
...
代码完成只是第一步，我们还需要给代码加上[单元测试](https://eggjs.org/zh-CN/core/unittest)。
