...
4. Agent 和普通 App Worker 提供的 API 不完全相同。想知道具体差异，请查看[框架文档](https://eggjs.org/zh-CN/advanced/framework)。

### Agent 的用法

你可以在应用或插件的根目录下的 `agent.js` 文件中实现你自己的逻辑（使用方法类似于[启动自定义](https://eggjs.org/zh-CN/basics/app-start)，只是入口参数为 agent 对象）。
...
Worker 进程处理用户请求和[定时任务](https://eggjs.org/zh-CN/basics/schedule)。Egg 的定时任务可控制仅由一个 Worker 进程执行，**因此可被解决的问题不应放在 Agent 上**。
...
在实际项目中，我们可以采用方案一作为基础。结合方案三或方案二中的一种，以提升数据更新的实时性。而在这个示例中，我们会使用 IPC + [定时任务](https://eggjs.org/zh-CN/basics/schedule)，同时实现这三种更新方案。
...
在上面的例子中，我们在 Agent 进程上运行了一个 subscriber，来监听消息中间件的消息。如果 Worker 进程也需要监听一些消息怎么办？如何通过 Agent 进程建立连接，再转发给 Worker 进程呢？这些问题可以在 [多进程模型增强](https://eggjs.org/zh-CN/advanced/cluster-client) 文档中找到答案。