...
在这里我们需要使用到 [egg-bin](https://github.com/eggjs/egg-bin) 模块（只在本地开发和单元测试使用，如果线上请参考 [应用部署](https://eggjs.org/zh-CN/core/deployment)）。
...
这里主要讲解工具部分的使用，更多关于单元测试的内容请参考[这里](https://eggjs.org/zh-CN/core/unittest)。
...
框架内置了 [日志](https://eggjs.org/zh-CN/core/logger) 功能，使用 `logger.debug()` 输出调试信息，**推荐在应用代码中使用它。**
...
`egg-bin` 会智能选择调试协议。在 Node.js 8.x 之后的版本中，使用 [Inspector Protocol](https://chromedevtools.github.io/debugger-protocol-viewer/v8) 协议，低版本使用 [Legacy Protocol](https://github.com/buggerjs/bugger-v8-client/blob/master/PROTOCOL.md)。
...
#### 使用 [DevTools](https://developer.chrome.com/devtools) 进行调试

最新的 DevTools 只支持 [Inspector Protocol](https://chromedevtools.github.io/debugger-protocol-viewer/v8) 协议，因此你需要使用 Node.js 8.x 及以上版本。
...
我们还提供了 [vscode-eggjs](https://github.com/eggjs/vscode-eggjs) 扩展，可以自动生成配置。
...
想要了解更多有关本地开发的内容，例如如何为你的团队定制本地开发工具，请参阅 [egg-bin](https://github.com/eggjs/egg-bin)。