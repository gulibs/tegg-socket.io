...
- 按插件 => 框架 => 应用的顺序依次加载。
- 插件之间的顺序由依赖关系决定，被依赖方先加载，无依赖者按 object key 的配置顺序加载。具体可以查看[插件章节](https://eggjs.org/zh-CN/advanced/plugin)。
- 框架按继承顺序加载，越底层越先加载。
...
1. 加载 [plugin](https://eggjs.org/zh-CN/advanced/plugin)，找到应用和框架，加载 `config/plugin.js`。
2. 加载 [config](https://eggjs.org/zh-CN/basics/config)，遍历 loadUnit 加载 `config/config.{env}.js`。
3. 加载 [extend](https://eggjs.org/zh-CN/basics/extend)，遍历 loadUnit 加载 `app/extend/xx.js`。
4. [自定义初始化](https://eggjs.org/zh-CN/basics/app-start)，遍历 loadUnit 加载 `app.js` 和 `agent.js`。
5. 加载 [service](https://eggjs.org/zh-CN/basics/service)，遍历 loadUnit 加载 `app/service` 目录。
6. 加载 [middleware](https://eggjs.org/zh-CN/basics/middleware)，遍历 loadUnit 加载 `app/middleware` 目录。
7. 加载 [controller](https://eggjs.org/zh-CN/basics/controller)，加载应用的 `app/controller` 目录。
8. 加载 [router](https://eggjs.org/zh-CN/basics/router)，加载应用的 `app/router.js`。

请注意：

- 加载时如果遇到同名文件将会被覆盖。比如，如果想要覆盖 `ctx.ip`，可以在应用的 `app/extend/context.js` 中直接定义 `ip`。
- 应用完整启动顺序请查看[框架开发](https://eggjs.org/zh-CN/advanced/framework)。
...
![img](https://user-images.githubusercontent.com/40081831/47344271-a688d500-d6da-11e8-96e9-663fa9f45108.png)
...
Loader 也提供了 [caseStyle](https://eggjs.org/zh-CN/advanced/loader#casestyle-string) 设置来强制指定命名方式，如将 model 加载时的 API 首字母大写，`app/model/user.js` => `app.model.User`，可指定 `caseStyle: 'upper'`。
...
以上只是说明 `Loader` 的写法，具体可以查看[框架开发](https://eggjs.org/zh-CN/advanced/framework)。
...
3. [`LoaderOptions`](https://eggjs.org/zh-CN/advanced/loader#loaderoptions) 包含了一些配置选项。
