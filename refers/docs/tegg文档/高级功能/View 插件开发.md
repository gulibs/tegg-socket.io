...
绝大多数情况下，我们都需要读取数据后渲染模板，然后呈现给用户。框架并不强制使用某种模板引擎，由开发者自行选型，具体参见[模板渲染](https://eggjs.org/zh-CN/core/view)。
...
- 遵循[插件开发规范](https://eggjs.org/zh-CN/advanced/plugin)。
- 插件命名约定以 `egg-view-` 开头。
...
模板与安全密不可分。[@eggjs/security](https://github.com/eggjs/security) 也为模板提供了一些方法。模板引擎可以根据需求使用这些方法。

首先声明对 [@eggjs/security](https://github.com/eggjs/security) 的依赖：
...
除此之外，框架还提供了 [app.injectCsrf](https://eggjs.org/zh-CN/core/security#appinjectcsrfstr) 与 [app.injectNonce](https://eggjs.org/zh-CN/core/security#appinjectnoncestr) 方法。更多内容可查看[安全章节](https://eggjs.org/zh-CN/core/security)。

### 单元测试

为了确保插件的高质量，完善的单元测试是不可或缺的。我们也提供了很多辅助工具，以帮助插件开发者毫无障碍地编写测试。具体内容请参见[单元测试](https://eggjs.org/zh-CN/core/unittest)与[插件](https://eggjs.org/zh-CN/advanced/plugin)相关章节。