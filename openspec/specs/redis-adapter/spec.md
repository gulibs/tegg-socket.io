# redis-adapter Specification

## Purpose
This specification defines the optional Redis adapter integration for the `@eggjs/tegg-socket.io` plugin, enabling cluster mode support. It ensures that the Redis adapter is properly loaded, configured, and attached to the Socket.IO server when cluster mode is enabled.
## Requirements
